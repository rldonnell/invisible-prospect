 ...
 *
 * Four Winds CMMS (B2B): page-view signals cap at Medium. The High tier is
 * reserved for visitors who return on 2+ distinct calendar dates (handled
 * downstream by detectReturnVisitor + the process-visitors promotion block).
 * HOT is reserved for visitors who CLICK on a Four Winds follow-up email
 * (the "warm leads" cohort) — same engagement-only HOT rule as other clients.
 */
...
// Clients whose page-view scoring is capped at Medium. The High tier is only
// reachable via the multi-day return-visitor promotion in process-visitors,
// and HOT is only reachable via an Instantly email click. Four Winds is a B2B
// CMMS lead nurture - research-mode visitors should not be flagged "High"
// just because they read a few feature pages in one sitting.
const MEDIUM_CAPPED_CLIENTS = new Set(['four-winds']);
 ...
 * @param {string} [clientKey] - Optional client key. Used to apply per-client
 *   tier ceilings (e.g. Four Winds caps at Medium).
 * @returns {{ score: number, tier: string }}
 */
export function scoreIntent(visitor, classifications, clientKey) {
  let score = 0;
  ...
  // Per-client cap: Medium-capped clients (e.g. Four Winds) cannot reach
  // High from page-view scoring alone. Multi-day return visitors are still
  // promoted to High by the process-visitors cron after this returns.
  if (clientKey && MEDIUM_CAPPED_CLIENTS.has(clientKey) && tier === "High") {
    tier = "Medium";
  }
